...
collection. Asserts every method's exact request URL."""
...
def initiatives(config: Configuration) -> Initiatives:
    return Initiatives(V2Transport(config))
...
def test_list_passes_expand(initiatives: Initiatives) -> None:
    ...
    initiatives.list("acme", expand=["lead"])
...
def test_list_rejects_unknown_expand_before_the_request(initiatives: Initiatives) -> None:
    with pytest.raises(ValueError, match="bogus"):
        initiatives.list("acme", expand=["bogus"])


@responses.activate
def test_iterate_initiatives(initiatives: Initiatives) -> None:
    responses.get(
        f"{BASE}/",
        json={"data": [{"id": "in1", "name": "Q3 push"}], "pagination": {"style": "offset"}},
    )

    rows = list(initiatives.iterate("acme"))
...
@responses.activate
def test_retrieve_initiative(initiatives: Initiatives) -> None:
    responses.get(f"{BASE}/in1/", json={"id": "in1", "name": "Q3 push"})

    row = initiatives.retrieve("acme", "in1")

    assert row.id == "in1"
    assert responses.calls[0].request.url == f"{BASE}/in1/"
...
def test_create_then_patch_then_delete_initiative(initiatives: Initiatives) -> None:
    ...
    created = initiatives.create("acme", CreateInitiative(name="Q3 push"))
    updated = initiatives.update("acme", created.id, UpdateInitiative(state="COMPLETED"))

    assert updated.state == "COMPLETED"
    assert initiatives.delete("acme", "in1") is None
    assert responses.calls[0].request.url == f"{BASE}/"
    assert responses.calls[1].request.url == f"{BASE}/in1/"
    assert responses.calls[2].request.url == f"{BASE}/in1/"
...
def test_find_initiative_by_name(initiatives: Initiatives) -> None:
    ...
    assert initiatives.find_by_name("acme", "Q3 push").id == "in1"
...
def test_work_items_add_sends_add_body_and_returns_added(initiatives: Initiatives) -> None:
    ...
    result = initiatives.work_items.add("acme", "in1", ["wi-1"])
...
def test_work_items_remove_sends_remove_body_and_returns_removed(initiatives: Initiatives) -> None:
    ...
    result = initiatives.work_items.remove("acme", "in1", ["wi-2"])
...
def test_work_items_bridge_rejects_empty_or_oversized_ids(initiatives: Initiatives) -> None:
    with pytest.raises(ValueError):
        initiatives.work_items.add("acme", "in1", [])
    with pytest.raises(ValueError):
        initiatives.work_items.add("acme", "in1", [f"wi-{i}" for i in range(101)])
    with pytest.raises(ValueError):
        initiatives.work_items.remove("acme", "in1", [])
    with pytest.raises(ValueError):
        initiatives.work_items.remove("acme", "in1", [f"wi-{i}" for i in range(101)])
...
def test_projects_add_sends_add_body_and_returns_added(initiatives: Initiatives) -> None:
    ...
    result = initiatives.projects.add("acme", "in1", ["proj-1"])
...
def test_projects_remove_sends_remove_body_and_returns_removed(initiatives: Initiatives) -> None:
    ...
    result = initiatives.projects.remove("acme", "in1", ["proj-2"])
...
def test_projects_bridge_rejects_empty_or_oversized_ids(initiatives: Initiatives) -> None:
    with pytest.raises(ValueError):
        initiatives.projects.add("acme", "in1", [])
    with pytest.raises(ValueError):
        initiatives.projects.add("acme", "in1", [f"proj-{i}" for i in range(101)])
    with pytest.raises(ValueError):
        initiatives.projects.remove("acme", "in1", [])
    with pytest.raises(ValueError):
        initiatives.projects.remove("acme", "in1", [f"proj-{i}" for i in range(101)])
...
def test_initiative_labels_bridge_add_sends_add_body_and_returns_added(
    ...
    result = initiatives.labels.add("acme", "in1", ["lbl-1"])
...
def test_initiative_labels_bridge_remove_sends_remove_body_and_returns_removed(
    ...
    result = initiatives.labels.remove("acme", "in1", ["lbl-2"])
...
def test_initiative_labels_bridge_rejects_empty_or_oversized_ids(
    ...
    with pytest.raises(ValueError):
        initiatives.labels.add("acme", "in1", [])
    with pytest.raises(ValueError):
        initiatives.labels.add("acme", "in1", [f"lbl-{i}" for i in range(101)])
    with pytest.raises(ValueError):
        initiatives.labels.remove("acme", "in1", [])
    with pytest.raises(ValueError):
        initiatives.labels.remove("acme", "in1", [f"lbl-{i}" for i in range(101)])
...
def test_initiative_labels_crud_has_no_initiative_id_in_its_path(
    ...
    responses.get(f"{BASE}/labels/lbl-1/", json={"id": "lbl-1", "name": "priority"})
    responses.patch(f"{BASE}/labels/lbl-1/", json={"id": "lbl-1", "color": "#f00"})
    responses.delete(f"{BASE}/labels/lbl-1/", status=204)

    page = initiatives.labels.list("acme")
    assert page.data[0].name == "priority"
    assert responses.calls[0].request.url == f"{BASE}/labels/"

    created = initiatives.labels.create("acme", CreateInitiativeLabel(name="priority"))
    assert created.id == "lbl-1"

    fetched = initiatives.labels.retrieve("acme", "lbl-1")
    assert fetched.id == "lbl-1"
    assert responses.calls[2].request.url == f"{BASE}/labels/lbl-1/"

    updated = initiatives.labels.update("acme", "lbl-1", UpdateInitiativeLabel(color="#f00"))
    assert updated.color == "#f00"
    assert responses.calls[3].request.url == f"{BASE}/labels/lbl-1/"

    assert initiatives.labels.delete("acme", "lbl-1") is None
    assert responses.calls[4].request.url == f"{BASE}/labels/lbl-1/"
...
def test_initiative_labels_find_by_name(initiatives: Initiatives) -> None:
    ...
    assert initiatives.labels.find_by_name("acme", "priority").id == "lbl-1"

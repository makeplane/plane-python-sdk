...
@pytest.fixture
def permission_schemes(client: PlaneClient, workspace_slug: str) -> PermissionSchemes:
    return client.v2.workspace(workspace_slug).permission_schemes


class TestPermissionSchemes:
    def test_list_includes_system_schemes(self, permission_schemes: PermissionSchemes) -> None:
    ...
    def test_retrieve_round_trips_a_listed_scheme(
        self, permission_schemes: PermissionSchemes
    ) -> None:
        first = permission_schemes.list().data[0]
        fetched = permission_schemes.retrieve(first.id)
        assert fetched.id == first.id

    def test_retrieve_unknown_id_is_404(self, permission_schemes: PermissionSchemes) -> None:
        with pytest.raises(PlaneAPIError) as exc_info:
            permission_schemes.retrieve("00000000-0000-0000-0000-000000000000")

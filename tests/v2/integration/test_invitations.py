...
def invitations(client: PlaneClient, workspace_slug: str) -> Invitations:
    return client.v2.workspace(workspace_slug).invitations


def test_list(invitations: Invitations) -> None:
    page = invitations.list()
    assert isinstance(page.data, list)


def test_create_retrieve_delete(invitations: Invitations) -> None:
    email = _throwaway_email("single")
    created = invitations.create(CreateWorkspaceInvite(email=email))
    try:
        fetched = invitations.retrieve(created.id)
        assert fetched.email == email
        assert fetched.accepted is False
    finally:
        invitations.delete(created.id)


def test_bulk_creates_every_new_email(invitations: Invitations) -> None:
    emails = [_throwaway_email("bulk-a"), _throwaway_email("bulk-b")]
    created = invitations.bulk(BulkCreateWorkspaceInvites(emails=emails))
    try:
        assert {row.email for row in created} == set(emails)
    finally:
        for row in created:
            try:
                invitations.delete(row.id)
            except Exception:
                pass


def test_bulk_skips_already_invited_emails(invitations: Invitations) -> None:
    ...
    first = invitations.create(CreateWorkspaceInvite(email=email))
    try:
        second = invitations.bulk(BulkCreateWorkspaceInvites(emails=[email]))
        assert second == []
    finally:
        invitations.delete(first.id)

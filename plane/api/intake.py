...
class Intake(BaseResource):
    ...
    def retrieve(
        ...
        """Retrieve an intake work item by ID.

        Args:
            workspace_slug: The workspace slug identifier
            project_id: UUID of the project
            intake_issue_id: UUID of the intake work item
            params: Optional query parameters for expand, fields, etc.
        """
        query_params = params.model_dump(exclude_none=True) if params else None
        response = self._get(
            f"{workspace_slug}/projects/{project_id}/intake-issues/{intake_issue_id}",
            params=query_params,
        )
    ...
    def update(
        ...
        """Update an intake work item by ID.
    ...
    def delete(self, workspace_slug: str, project_id: str, intake_issue_id: str) -> None:

...
from pydantic import BaseModel, Field, StrictBool, StrictBytes, StrictFloat, StrictInt, StrictStr, conint, conlist, constr, validator
...
class IssueExpand(BaseModel):
    ...
    priority: Optional[StrictStr] = Field(default=None, description="* `urgent` - Urgent * `high` - High * `medium` - Medium * `low` - Low * `none` - None")
    ...
    def priority_validate_enum(cls, value):
        ...
        if value not in ('urgent', 'high', 'medium', 'low', 'none',):
            raise ValueError("must be one of enum values ('urgent', 'high', 'medium', 'low', 'none')")
        return value
